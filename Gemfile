source 'https://rubygems.org'
source 'https://rails-assets.org'
ruby '2.1.2'

gem 'rails', '4.1.0'

gem 'pg'
gem 'mysql2'

gem 'turbolinks'
gem 'jbuilder', '~> 2.0'

gem 'active_link_to'
gem 'activeadmin', github: 'gregbell/active_admin'
gem 'aws-sdk'
gem 'before_render', github: 'lightyrs/before_render'
gem 'bitly', github: 'lightyrs/bitly', branch: 'v3_api_generic_oauth2_access_token'
gem 'cancan'
gem 'carrierwave'
gem 'ckeditor'
gem 'colored'
gem 'compass-rails'
gem 'devise'
gem 'dotiw', github: 'rarestep/dotiw', branch: 'rails4'
gem 'fastercsv'
gem 'fog'
gem 'friendly_id'
gem 'gabba'
gem 'garails'
gem 'gon', git: 'git://github.com/gazay/gon.git'
gem 'gretel'
gem 'impressionist'
gem 'koala'
gem 'mini_magick'
gem 'newrelic_rpm'
gem 'omniauth'
gem 'omniauth-facebook', git: 'git://github.com/mkdynamic/omniauth-facebook.git'
gem 'sanitize'
gem 'protected_attributes'
gem 'pundit'
gem 'sidekiq'
gem 'simple_form'
gem 'simple_uuid'
gem 'sinatra', require: nil
gem 'slim'
gem 'stripe-rails'
gem 'stripe_tester'
gem 'validates_timeliness', '~> 3.0'
gem 'whenever', require: false
gem 'will_paginate-bootstrap'
gem 'x-editable-rails', github: 'lightyrs/x-editable-rails'

# Assets
gem 'jquery-turbolinks'
gem 'font-awesome-sass'
gem 'handlebars_assets'
gem 'rails-assets-bourbon', '~> 3.2.0'
gem 'rails-assets-jquery', '~> 2.0.3'
gem 'rails-assets-jquery-autosize'
gem 'rails-assets-jquery-flot'
gem 'rails-assets-list.js'
gem 'rails-assets-lodash'
gem 'rails-assets-moment'
gem 'ajax-datatables-rails', git: 'git://github.com/CyborgMaster/ajax-datatables-rails.git'
gem 'autoprefixer-rails'
gem 'haml-rails'
gem 'less-rails'
gem 'sass-rails', '~> 4.0.3'
gem 'uglifier', '>= 1.3.0'
gem 'coffee-rails', '~> 4.0.0'
gem 'zeroclipboard-rails'
gem 'therubyracer', platforms: :ruby

group :development do
  gem 'capistrano', '~> 3.2.0'
  gem 'capistrano-bundler'
  gem 'capistrano-rails'
  gem 'capistrano-rails-console'
  gem 'capistrano-rvm'
  gem 'html2haml'
  gem 'rails_apps_pages'
  gem 'rails_apps_testing'
  gem 'rails_layout'
  gem 'spring'
end

group :development, :test do
  gem 'rails_best_practices'
  gem 'awesome_print'
  gem 'better_errors'
  gem 'binding_of_caller', platforms: [:mri_21]
  gem 'chrome_logger' , :require => 'chrome_logger/railtie'
  gem 'meta_request'
  gem 'erb2haml'
  gem 'factory_girl_rails'
  gem 'factory_girl_sequences'
  gem 'ffaker'
  gem 'hirb'
  gem 'interactive_editor'
  gem 'magic_encoding'
  gem 'pry'
  gem 'pry-remote'
  gem 'pry-coolline'
  gem 'pry-rails'
  gem 'pry-rescue'
  gem 'rspec-rails'
  gem 'quiet_assets'
  gem 'rubygems-bundler'
  gem 'seed_dump'
  gem 'dotenv-rails'
  gem 'foreman'
  gem 'thin'
end

group :production do
  gem 'unicorn'
end

group :test do
  gem 'launchy'
  gem 'capybara'
  gem 'database_cleaner'
  gem 'selenium-webdriver'
end

group :doc do
  gem 'sdoc', '~> 0.4.0'
end